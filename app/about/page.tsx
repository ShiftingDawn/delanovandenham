    ...
    return (
        <Page name={"About me"}>
            <InfoChip icon={<HugeiconsIcon icon={OfficeIcon}/>}
                      text={"Looking for opportunities in Utrecht (the Netherlands)"}/>
...
function InfoChip({icon, text}: Readonly<{ icon: ReactNode, text: string }>) {
    return (
        <div
            className={"bg-ctp-surface0 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-ctp-lavender hover:text-ctp-base transition-colors"}>
            {icon}
            {text}
        </div>
    );
...
function MarkedText({children}: FCP) {
    return <span className={"text-ctp-lavender font-bold"}>{children}</span>;
}